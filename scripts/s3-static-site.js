...
module.exports = {
  async deploy({bucket, cloudFrontId, profile}) {
    ...
    await execAsync(`aws s3 rm s3://${bucket}/typescript-intro --recursive --profile=${profile || defaultProfile}`, {cwd: projectRoot});
    await execAsync(`aws s3 cp dist s3://${bucket}/typescript-intro/ --recursive --profile=${profile || defaultProfile}`, {cwd: projectRoot});
    await execAsync(`aws cloudfront create-invalidation --distribution-id ${cloudFrontId}  --paths /typescript-intro/\\* --profile=${profile || defaultProfile}`, {cwd: projectRoot});
  }
};